...
function refreshFileManagerImages() {
    $.ajax({
        type: 'POST',
        url: generateUrl('File/getBlogImages'),
        data: {},
        success: function (response) {
            if (response.result == 1) {
                document.getElementById("image_file_manager_upload_response").innerHTML = response.content;
            }
        }
    });
}
...
$(document).on('click', '#imageFileManagerModal #btn_file_manager_delete', function () {
    ...
    $.ajax({
        type: 'POST',
        url: generateUrl('File/deleteBlogImage'),
        data: {'file_id': fileId},
        success: function (response) {
            $('#btn_file_manager_delete').hide();
            $('#btn_file_manager_select').hide();
...
$(document).on('click', '#btn_delete_blog_main_image', function () {
    var content = '<a class="btn-select-image" data-toggle="modal" data-target="#imageFileManagerModal">' +
        ...
        '<button class="btn">' + MdsConfig.text.selectImage + '</button>' +
        '</div>' +
        '</a>';
...
$(document).on('click', '#btn_delete_blog_main_image_database', function () {
    ...
    $.ajax({
        type: 'POST',
        url: generateUrl('Blog/deletePostImagePost'),
        data: data,
        success: function (response) {
            var content = '<a class="btn-select-image" data-toggle="modal" data-target="#imageFileManagerModal">' +
                '<div class="btn-select-image-inner">' +
...
jQuery(function ($) {
    $('.file-manager-content').on('scroll', function () {
        if ($(this).scrollTop() + $(this).innerHeight() >= $(this)[0].scrollHeight) {
            ...
            $.ajax({
                type: 'POST',
                url: generateUrl('File/loadMoreBlogImages'),
                data: data,
                success: function (response) {
                    setTimeout(function () {
                        if (response.result == 1) {
                            $("#image_file_manager_upload_response").append(response.content);
                        }
                    }, 100);
                }
            });
