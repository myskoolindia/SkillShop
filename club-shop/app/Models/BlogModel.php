...
class BlogModel extends BaseModel
{
    ...
    {
        return $this->builder->select('blog_posts.*, blog_categories.name AS category_name, blog_categories.slug AS category_slug')->
        join('blog_categories', 'blog_posts.category_id = blog_categories.id')->where('blog_posts.id', clrNum($id))->get()->getRow();
    }
    ...
    //get posts
    public function getPosts($langId, $limit)
    {
        return getCacheData('blog_posts_lang_' . $langId . '_limit_' . $limit, function () use ($langId, $limit) {
            $this->builQuery($langId);
    ...
    //get paginated sitemap posts
    public function getSitemapPostsPaginated($perPage, $offset)
    {
        return $this->builder->join('blog_categories', 'blog_posts.category_id = blog_categories.id')
            ->select('blog_posts.*, blog_categories.slug AS category_slug')->limit($perPage, $offset)->get()->getResult();
    }
    ...
    //get paginated sitemap blog categories
    public function getSitemapBlogCategoriesPaginated($perPage, $offset)
    {
        return $this->builderCategories->limit($perPage, $offset)->get()->getResult();
    }
    ...
    {
        return $this->builderComments->select('blog_comments.*, users.avatar AS user_avatar, users.storage_avatar AS user_storage_avatar')
            ->join('users', 'users.id = blog_comments.user_id', 'left')->where('blog_comments.post_id', clrNum($postId))->where('blog_comments.status', 1)
            ->orderBy('blog_comments.created_at', 'DESC')->limit(clrNum($limit))->get()->getResult();
    }
    ...
    //post active comment count
    public function getActiveCommentsCountByPostId($postId)
    {
        return $this->builderComments->where('post_id', clrNum($postId))->where('status', 1)->countAllResults();
    }
