<?php
/**
 * Shared plan cart partial.
 * Expected variables injected by the including view:
 *   $planKey   – 'basic' | 'advance' | 'premium'
 *   $planLabel – 'Basic' | 'Advance' | 'Premium'
 *
 * Plus the normal cart variables from CartController:
 *   $cart, $userSession, $generalSettings, $productSettings
 */

$planColors = [
    'basic'   => ['bg' => '#f0fdf4', 'border' => '#86efac', 'text' => '#166534', 'badge_bg' => '#dcfce7', 'badge_text' => '#15803d'],
    'advance' => ['bg' => '#eff6ff', 'border' => '#93c5fd', 'text' => '#1e40af', 'badge_bg' => '#dbeafe', 'badge_text' => '#1d4ed8'],
    'premium' => ['bg' => '#fdf4ff', 'border' => '#d8b4fe', 'text' => '#6b21a8', 'badge_bg' => '#f3e8ff', 'badge_text' => '#7e22ce'],
];
$planIcons = [
    'basic'   => 'fa-cube',
    'advance' => 'fa-cubes',
...
$c  = $planColors[$planKey]  ?? $planColors['basic'];
$ic = $planIcons[$planKey]   ?? 'fa-cube';
?>
    ...
    <div class="container">
        <div class="row">
            <div class="col-12">
                ...
                <!-- Plan banner -->
                <div style="background:<?= $c['bg'] ?>; border:1px solid <?= $c['border'] ?>; border-radius:10px; padding:14px 20px; margin:24px 0 8px; display:flex; align-items:center; gap:12px;">
                    <span style="background:<?= $c['badge_bg'] ?>; color:<?= $c['badge_text'] ?>; font-size:13px; font-weight:700; padding:4px 12px; border-radius:20px; border:1px solid <?= $c['border'] ?>; white-space:nowrap;">
                        <i class="fa <?= $ic ?>"></i>&nbsp; <?= esc($planLabel) ?> Plan
                    </span>
                    <span style="font-size:14px; color:<?= $c['text'] ?>; font-weight:600;">
                        You are checking out the <strong><?= esc($planLabel) ?></strong> plan. Review your items below.
                    </span>
                </div>
...
.shopping-cart .cart-item-card .number-spinner { width: 116px; height: 38px; }
.shopping-cart .cart-item-card .number-spinner .input-group { height: 38px; }
.shopping-cart .cart-item-card .number-spinner button {
    height: 38px; width: 34px; padding: 0; line-height: 36px;
    border: 1px solid #cbd5e1; background: #f8fafc !important;
    color: #334155; font-weight: 700;
}
.shopping-cart .cart-item-card .number-spinner button:hover { background: #e2e8f0 !important; }
.shopping-cart .cart-item-card .number-spinner input {
    height: 38px; font-size: 14px; font-weight: 700;
    border-top: 1px solid #cbd5e1; border-bottom: 1px solid #cbd5e1;
    border-left: none; border-right: none;
    padding: 0 4px !important; color: #1e293b;
}
@media (max-width: 767.98px) {
    .shopping-cart .cart-item-card { padding: 14px; }
    .shopping-cart .cart-item-top-row { flex-direction: column; align-items: flex-start !important; }
        ...
        margin-top: 12px; padding-top: 12px; border-top: 1px dashed #e2e8f0;
                                    ...
                                    <?php if (!empty($cart->items)):
                                        ...
                                        <div class="item cart-item-card">
                                            <!-- Top Row -->
                                            <div class="cart-item-top-row d-flex flex-wrap align-items-center justify-content-between"
                                                 style="display:flex; flex-wrap:wrap; align-items:center; justify-content:space-between; gap:16px;
                                                        padding-bottom:<?= (!empty($cartItem->is_bundle) && (!empty($cartItem->bundle_categories) || !empty($cartItem->bundle_products))) ? '14px' : '0' ?>;
                                                        <?= (!empty($cartItem->is_bundle) && (!empty($cartItem->bundle_categories) || !empty($cartItem->bundle_products))) ? 'border-bottom:1px solid #f1f5f9;' : '' ?>">
                                                ...
                                                <div class="d-flex align-items-center" style="display:flex; align-items:center; gap:16px; flex:1 1 320px; min-width:0;">
                                                        ...
                                                        <a href="<?= esc($cartItem->product_url); ?>">
                                                            <div class="product-image-box product-image-box-sm" style="width:80px; height:80px; border-radius:8px; overflow:hidden; border:1px solid #e2e8f0;">
                                                                <img src="<?= getOrderImageUrl($cartItem->product_image_data, $cartItem->product_id); ?>"
                                                                     data-src="<?= getOrderImageUrl($cartItem->product_image_data, $cartItem->product_id); ?>"
                                                    ...
                                                    <div class="cart-item-details">
                                                        ...
                                                        <?php if (!empty($cartItem->product_options_summary)): ?>
                                                            <div class="product-variant-info m-b-4"><?= $cartItem->product_options_summary; ?></div>
                                                        <?php endif; ?>
                                                        ...
                                                        <div class="cart-item-pricing-row d-flex flex-wrap align-items-center" style="display:flex; flex-wrap:wrap; align-items:center; gap:8px 16px;">
                                                            <?php if ($cartItem->purchase_type != 'bidding'): ?>
                                                                <div class="d-inline-flex align-items-center" style="display:inline-flex; align-items:center; gap:6px;">
                                                                    ...
                                                                    <span style="white-space:nowrap;">
                                                                        ...
                                                                        <?php endif; ?>
                                                                        <strong class="lbl-price" style="font-size:16px; font-weight:700; color:#1e293b;"><?= priceDecimal($cartItem->unit_price, $cart->currency_code); ?></strong>
                                                                    </span>
                                                                </div>
                                                            ...
                                                            <div class="d-inline-flex align-items-center" style="display:inline-flex; align-items:center; gap:6px;">
                                                                ...
                                                                <?php if (!empty($cartItem->has_discount) && !empty($cartItem->discount_amount) && $cartItem->discount_amount > 0): ?>
                                                                    <span class="badge badge-success" style="background:#dcfce7; color:#15803d; font-size:11px; font-weight:700; border:1px solid #bbf7d0; padding:2px 6px; border-radius:4px; margin-left:4px;">
                                                                        Save <?= priceDecimal($cartItem->discount_amount, $cart->currency_code); ?>
                                                                    </span>
                                                                <?php endif; ?>
                                                            </div>
                                                        </div>
                                            ...
                                            <?php if (!empty($cartItem->is_bundle) && (!empty($cartItem->bundle_categories) || !empty($cartItem->bundle_products))): ?>
                                                <div class="bundle-items-container m-t-15 p-3" style="width:100%; background:#f8fafc; border:1px solid #e2e8f0; border-radius:8px;">
                                                    <div class="d-flex justify-content-between align-items-center m-b-12 pb-2" style="border-bottom:1px solid #e2e8f0; display:flex; justify-content:space-between; align-items:center;">
                                                        ...
                                                        <a href="<?= esc($cartItem->product_url); ?>?cart_item_id=<?= $cartItem->id; ?>&return=<?= esc(base_url($planKey . '-cart')); ?>#tab_bundle_contents"
                                                           class="btn btn-sm btn-outline-primary" style="font-size:12px; padding:3px 10px; border-radius:4px; font-weight:600;">
                                                        ...
                                                        <?php foreach ($cartItem->bundle_categories as $bCategory): ?>
                                                            <div class="bundle-category-section m-b-12" style="background:#ffffff; border:1px solid #e2e8f0; border-radius:8px; overflow:hidden; box-shadow:0 1px 2px rgba(0,0,0,0.03);">
                                                                <div class="d-flex justify-content-between align-items-center px-3 py-2" style="background:#f8fafc; border-bottom:1px solid #e2e8f0; display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:8px;">
                                                                    ...
                                                                    <div style="background:#eff6ff; border:1px solid #bfdbfe; border-radius:6px; padding:3px 10px; display:inline-flex; align-items:center; gap:6px;">
                                                                                ...
                                                                                <tr style="border-bottom:1px solid #f8fafc;">
                                                                                    <td style="padding:8px 12px; vertical-align:middle;">
                                                                                        <div style="display:flex; align-items:center;">
                                                                                            ...
                                                                                            <div>
                                                                                                <a href="<?= esc($bProd->product_url); ?>" style="color:#1e293b; font-weight:500; text-decoration:none;"><?= esc($bProd->title); ?></a>
                                                                                                <?php if (!empty($bProd->sku)): ?><br><small class="text-muted" style="font-size:11px;">SKU: <?= esc($bProd->sku); ?></small><?php endif; ?>
                                                                                            </div>
                                                                                        </div>
                                                                                    </td>
                                                                                    <td class="text-center" style="padding:8px 12px; vertical-align:middle; color:#475569;">
                                                                                        <?php if (!empty($bProd->has_discount)): ?><del style="font-size:11px; color:#94a3b8; margin-right:3px;"><?= priceDecimal($bProd->original_price, $cart->currency_code); ?></del><?php endif; ?>
                                                                                        <span style="font-weight:600; color:#334155;"><?= priceDecimal($bProd->unit_price, $cart->currency_code); ?></span>
                                                                                    </td>
                                                                                    <td class="text-center" style="padding:8px 12px; vertical-align:middle; font-weight:600; color:#1e293b;"><?= $bProd->quantity; ?></td>
                                                                                    <td class="text-right font-weight-bold" style="padding:8px 12px; vertical-align:middle; color:#2563eb;">
                                                                                        <?php if (!empty($bProd->has_discount)): ?><del style="font-size:11px; color:#94a3b8; font-weight:400; margin-right:3px;"><?= priceDecimal($bProd->original_total_price, $cart->currency_code); ?></del><?php endif; ?>
                                                                        ...
                                                                        <tr style="border-bottom:1px solid #edf2f7;">
                                                                            <td style="padding:8px 10px; vertical-align:middle;">
                                                                                <div style="display:flex; align-items:center;">
                                                                                    <img src="<?= getProductMainImage($bProd->product_id, 'image_small'); ?>" alt="<?= esc($bProd->title); ?>" style="width:36px; height:36px; object-fit:cover; border-radius:4px; margin-right:10px; border:1px solid #e2e8f0; flex-shrink:0;">
                                                                                    <div>
                                                                                        <a href="<?= esc($bProd->product_url); ?>" style="color:#2d3748; font-weight:500; text-decoration:none;"><?= esc($bProd->title); ?></a>
                                                                                        <?php if (!empty($bProd->sku)): ?><br><small class="text-muted" style="font-size:11px;">SKU: <?= esc($bProd->sku); ?></small><?php endif; ?>
                                                                                    </div>
                                                                                </div>
                                                                            </td>
                                                                            ...
                                                                            <td class="text-center" style="padding:8px 10px; vertical-align:middle; font-weight:600; color:#2d3748;"><?= $bProd->quantity; ?></td>
                            ...
                            <div class="col-sm-12 col-lg-4">
                                <div class="right">
                                    ...
                                    <!-- Plan label in sidebar -->
                                    <div style="background:<?= $c['badge_bg'] ?>; border:1px solid <?= $c['border'] ?>; border-radius:8px; padding:10px 14px; margin-bottom:18px; text-align:center;">
                                        <span style="font-size:13px; font-weight:700; color:<?= $c['badge_text'] ?>;">
                                            <i class="fa <?= $ic ?>"></i>&nbsp; <?= esc($planLabel) ?> Plan — Order Summary
                                        </span>
                                    </div>
                                    ...
                                    <?php if (!empty($cart->totals->total_savings) && $cart->totals->total_savings > 0): ?>
                                        <div class="row-custom m-b-15" style="background:#f0fdf4; border:1px dashed #86efac; border-radius:8px; padding:10px 12px;">
                                            <div style="display:flex; justify-content:space-between; align-items:center;">
                                                <strong style="color:#166534; font-size:14px;"><i class="fa fa-tag text-success m-r-1"></i> Total Savings</strong>
                                                <strong class="text-success" style="font-size:15px; font-weight:800; color:#15803d !important;">- <?= priceDecimal($cart->totals->total_savings, $cart->currency_code); ?></strong>
                                            </div>
                                            <?php if (!empty($cart->totals->savings_percentage) && $cart->totals->savings_percentage > 0): ?>
                                                <div style="font-size:11px; color:#16a34a; font-weight:600; margin-top:3px;">You are saving <?= $cart->totals->savings_percentage; ?>% on this order!</div>
                                            <?php endif; ?>
                                        </div>
                                    <?php endif; ?>
                                    ...
                                    <div class="row-custom"><p class="line-seperator"></p></div>
                                    ...
                                    <div class="row-custom m-t-30 m-b-15">
                                        ...
                                        <?php else:
                                            ...
                                            } elseif ($generalSettings->guest_checkout == 1 && !$cart->has_digital_product) {
                                                $showCheckoutBtn = true;
                                            }
                                            if ($showCheckoutBtn): ?>
                                    ...
                                    <div class="cart-coupon-error">
                                        <?php if (!empty(helperGetSession('error_coupon_code'))): ?>
                                            <div class="text-danger"><?= helperGetSession('error_coupon_code'); ?></div>
                                        <?php endif; ?>
                                    </div>
