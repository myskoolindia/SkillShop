<div id="wrapper">
                                    ...
                                    <ul>
                                        <?php foreach ($socialLinks as $socialLink):
                                            if (!empty($socialLink['value'])): ?>
                                                <li><a href="<?= esc($socialLink['value']); ?>" target="_blank"><i class="icon-<?= esc($socialLink['name']); ?>"></i></a></li>
                                            <?php endif;
                                        endforeach; ?>
                                    </ul>
                                <?php endif; ?>
    ...
    <?php if (!empty($baseSettings->contact_address)):
        $mapLang = 'en';
        if (!empty($activeLang->short_form)) {
            $mapLang = $activeLang->short_form;
        } ?>
        <div class="container-fluid">
            <div class="row">
                <div class="contact-map-container">
                    <iframe id="contact_iframe" src="https://maps.google.com/maps?width=100%&height=600&hl=<?= $mapLang; ?>&q=<?= urlencode($baseSettings->contact_address); ?>&ie=UTF8&t=&z=8&iwloc=B&output=embed&disableDefaultUI=true" frameborder="0" scrolling="no" marginheight="0" marginwidth="0"></iframe>
                </div>
            </div>
        </div>
    <?php endif; ?>
