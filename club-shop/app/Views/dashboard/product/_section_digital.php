...
<?php $allowedExts = trim($productSettings->digital_allowed_file_extensions ?? '', '"'); ?>

<script>
    function initDigitalFileUploader() {
        $('#drag-and-drop-zone-digital-files').dmUploader({
            url: '<?= base_url('upload-digital-file-post'); ?>',
            queue: true,
            extFilter: [<?= implode(',', array_map(fn($v) => '"' . trim($v) . '"', explode(',', $allowedExts))); ?>],
            multiple: false,
            ...
            onUploadSuccess: function (id, data) {
                if (data.result == 1) {
                    document.getElementById("digital_files_upload_result").innerHTML = data.htmlContent;
                }
            },
