        ...
        <ul class="dm-uploaded-files" id="files-image">
            ...
            if (!empty($images)):
                foreach ($images as $image):
                    ...
                    $imgTempUrl = str_starts_with($image->image_small, 'uploads/temp/') ? base_url($image->image_small) : base_url('uploads/temp/' . $image->image_small); ?>
    ...
    function initImageUploader() {
        $('#drag-and-drop-zone').dmUploader({
            ...
            onUploadSuccess: function (id, data) {
                ...
                var fileId = (data && data.file_id !== undefined) ? data.file_id : id;
                loadImagePreview(id, fileId);
            },
    ...
    function loadImagePreview(id, fileId) {
        $.ajax({
            ...
            success: function (response) {
                ...
                if (response && response.result == 1 && response.imageHtml) {
                    var el = document.getElementById("uploaderFile" + id);
                    if (el) {
                        el.innerHTML = response.imageHtml;
                    }
                }
            }
        });
