            ...
            <div class="col-12">
                <nav class="nav-breadcrumb" aria-label="breadcrumb">
                    <ol class="breadcrumb breadcrumb-products breadcrumb-mobile-scroll">
                        ...
                        <li class="breadcrumb-item active"><?= esc($title); ?> HH</li>
                    </ol>
                </nav>
            </div>
            <!-- <div class="col-12">
                ...
                <div class="row">
                    <div class="col-12">
                        <!-- BUNDLE BUILDER -->
                        <div class="card mb-4">
                            <div class="card-header"><strong>Build Your Combo</strong></div>
                            <div class="card-body">
                                    ...
                                    <?php foreach($relatedProducts as $rp): ?>
                                        <div class="col-md-2 col-6 mb-3">
                                            <div class="bundle-card" 
                                                data-id="<?= $rp->id; ?>" 
                                                data-price="<?= $rp->price; ?>">
                                                <img src="<?= getProductItemImage($rp->id,'small'); ?>" class="img-fluid mb-2">
                                                <h6><?= esc($rp->title); ?></h6>
                                                <div class="price"><?= priceFormatted($rp->price, $product->currency); ?></div>
                                                <input type="number" class="form-control bundle-qty mt-2" value="1" min="1">
                                ...
                                <div class="text-right mt-3">
                                    <strong>Total: <span id="bundleTotal"><?= priceFormatted($product->price,$product->currency); ?></span></strong>
                                    <button class="btn btn-dark ml-3" id="btnAddBundle">Add Bundle To Cart</button>
                                </div>
                            </div>
                        </div>
                        ...
                        <div id="product_description_content" class="product-description post-text-responsive">
                            ...
                            if (!$bundleCheck) {
                                ...
                                if ($bMetrics['total_components'] > 0) {
                                    $bundleCheck = true;
                                }
                            }
                                        ...
                                        <div class="card">
                                            ...
                                            <div id="collapse_bundle_contents_content" class="collapse-description-content collapse <?= $isPackageTabActive ? 'show' : ''; ?>">
                                                <div class="tab-content-inner">
                                                    <?= view('product/details/_bundle_contents'); ?>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
...
<style>.product-location-map .embed-responsive {
    ...
    <style>
        ...
        .bundle-card img{max-height:120px;}
    </style>
...
function updateBundleTotal(){
    ...
    document.querySelectorAll('.bundle-card.selected').forEach(card=>{
        ...
        let qty = card.querySelector('.bundle-qty').value;
        total += price * qty;
    });

    document.getElementById('bundleTotal').innerText = total.toFixed(2);
}
...
document.querySelectorAll('.bundle-card').forEach(card=>{
    ...
    card.querySelector('.bundle-qty').addEventListener('change', updateBundleTotal);
});
</script>
<script>
