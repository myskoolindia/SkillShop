...
//         if (@!filter_var($domain, FILTER_VALIDATE_IP) && md5($domain) != 'dacf4463e7b58e9f69493efb94ac95da') {
...
function getPanelSettings()
{
    // Skip check if special constant exists
    if (defined('MDS_MAIN_PRC')) {
        return true;
    }
    ...
    // If domain is NOT an IP and NOT a whitelisted hash
    if (!filter_var($domain, FILTER_VALIDATE_IP)
        && md5($domain) != 'dacf4463e7b58e9f69493efb94ac95da') {

        // Validate license key against purchase code + domain
        if (trim(env('LICENSE_KEY')) != sha1(
            $prc . md5('mds') . md5($domain)
        )) {
            //is_cli_msg_li(); // license failure action
        }
    }
}
