...
{{-- =====================================================================
     CBSE 12 SKILL SECTORS — 3 FORMS OF WORK OVERVIEW
     ===================================================================== --}}
<section class="py-16 lg:py-20 bg-white">
  <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-3 items-start">
      ...
      {{-- Column 1: Life Forms --}}
      <div class="rounded-xl border-2 border-[#c7d4f5] bg-[#e8eeff] overflow-hidden mt-6">
        <div class="bg-[#c7d4f5] px-5 py-3">
          <span class="font-bold text-[#2b3a8f] text-sm">Life Forms</span>
        </div>
        <ul class="divide-y divide-[#d6e0f7]">
          @foreach([
            ['icon'=>'fa-leaf',     'label'=>'Agriculture &amp; Gardening', 'items'=>['Soil preparation & composting','Crop cultivation & harvesting','Greenhouse management','Drip irrigation systems','Organic farming practices']],
            ['icon'=>'fa-utensils', 'label'=>'Food Production',             'items'=>['Kitchen lab setup','Food safety & hygiene','Baking & culinary arts','Food packaging & preservation','Nutrition science']],
          ] as $i => $item)
          <li class="skill-accordion-item group">
            <button type="button"
                    class="skill-acc-btn w-full flex items-center justify-between px-5 py-3.5 hover:bg-[#dce6ff] transition-colors text-left">
              <div class="flex items-center gap-3">
                <span class="w-8 h-8 rounded-full bg-white flex items-center justify-center flex-shrink-0">
                  <i class="fa {{ $item['icon'] }} text-[#4a6cf7] text-sm"></i>
                </span>
                <span class="text-sm font-semibold text-slate-700">{!! $item['label'] !!}</span>
              </div>
              <i class="fa fa-chevron-down text-slate-400 text-xs skill-acc-icon transition-transform duration-300"></i>
            </button>
            <div class="skill-accordion-body overflow-hidden max-h-0 transition-[max-height] duration-300 ease-in-out bg-white border-t border-[#d6e0f7]">
              <ul class="space-y-1.5 px-5 py-3">
                @foreach($item['items'] as $sub)
                <li class="flex items-center gap-2 text-xs text-slate-600">
                  <span class="w-1.5 h-1.5 rounded-full bg-[#4a6cf7] flex-shrink-0"></span>{{ $sub }}
                </li>
                @endforeach
              </ul>
            </div>
      ...
      {{-- Column 2: Machines & Materials (center — raised up) --}}
      <div class="rounded-xl border-2 border-[#f4a261] bg-[#fde8d8] overflow-hidden shadow-lg -mt-2">
        <div class="bg-[#f4742b] px-5 py-3">
          <span class="font-bold text-white text-sm">Machines &amp; Materials</span>
        </div>
        <ul class="divide-y divide-[#f5caa8]">
          @foreach([
            ['icon'=>'fa-bolt',    'label'=>'Electronics / Mechatronics',    'items'=>['Circuit design & PCB assembly','Arduino & Raspberry Pi projects','Sensors & actuators','Motor control systems','Industrial automation basics']],
            ['icon'=>'fa-building','label'=>'Infrastructure &amp; Furniture', 'items'=>['Interior design basics','Plumbing & electrical safety','Construction materials','Furniture design & assembly','Space planning']],
            ['icon'=>'fa-hammer',  'label'=>'Carpentry / Woodwork',           'items'=>['Hand tools & power tools','Wood joinery techniques','Furniture making','Finishing & polishing','Safety in the workshop']],
            ['icon'=>'fa-desktop', 'label'=>'IT / ITeS Computer Lab',        'items'=>['Programming (Python, Scratch)','Web design & development','Cybersecurity basics','Data entry & office tools','Cloud computing fundamentals']],
            ['icon'=>'fa-car',     'label'=>'Automotive Workshop',            'items'=>['Engine components & maintenance','EV technology basics','Tyre & brake servicing','Vehicle safety systems','Diagnostic tools usage']],
            ['icon'=>'fa-film',    'label'=>'AVGC &amp; Media Studio',        'items'=>['Photography & videography','Audio recording & editing','Animation & motion graphics','Social media content creation','Podcast production']],
          ] as $item)
          <li class="skill-accordion-item">
            <button type="button"
                    class="skill-acc-btn w-full flex items-center justify-between px-5 py-3.5 hover:bg-[#f9d5b8] transition-colors text-left">
              <div class="flex items-center gap-3">
                <span class="w-8 h-8 rounded-full bg-white flex items-center justify-center flex-shrink-0">
                  <i class="fa {{ $item['icon'] }} text-[#f4742b] text-sm"></i>
                </span>
                <span class="text-sm font-semibold text-slate-700">{!! $item['label'] !!}</span>
              </div>
              <i class="fa fa-chevron-down text-slate-400 text-xs skill-acc-icon transition-transform duration-300"></i>
            </button>
            <div class="skill-accordion-body overflow-hidden max-h-0 transition-[max-height] duration-300 ease-in-out bg-white border-t border-[#f5caa8]">
              <ul class="space-y-1.5 px-5 py-3">
                @foreach($item['items'] as $sub)
                <li class="flex items-center gap-2 text-xs text-slate-600">
                  <span class="w-1.5 h-1.5 rounded-full bg-[#f4742b] flex-shrink-0"></span>{{ $sub }}
                </li>
                @endforeach
              </ul>
            </div>
      ...
      {{-- Column 3: Human Services --}}
      <div class="rounded-xl border-2 border-[#c7d4f5] bg-[#e8eeff] overflow-hidden mt-6">
        <div class="bg-[#c7d4f5] px-5 py-3">
          <span class="font-bold text-[#2b3a8f] text-sm">Human Services</span>
        </div>
        <ul class="divide-y divide-[#d6e0f7]">
          @foreach([
            ['icon'=>'fa-heart-pulse', 'label'=>'Healthcare',                    'items'=>['First aid & emergency response','Anatomy & physiology basics','Medical equipment handling','Patient care fundamentals','Health & wellness promotion']],
            ['icon'=>'fa-spa',         'label'=>'Beauty &amp; Wellness',          'items'=>['Personal care techniques','Skin & hair care science','Wellness practice routines','Grooming tools & safety','Hygiene & sanitation standards']],
            ['icon'=>'fa-shirt',       'label'=>'Apparel &amp; Fashion Design',   'items'=>['Fabric selection & textile science','Stitching & pattern making','Fashion illustration','Garment construction','Trend forecasting basics']],
            ['icon'=>'fa-landmark',    'label'=>'Retail / BFSI Simulation',       'items'=>['Customer service skills','Banking & financial transactions','Retail store management','Insurance basics','Digital payments & UPI']],
          ] as $item)
          <li class="skill-accordion-item">
            <button type="button"
                    class="skill-acc-btn w-full flex items-center justify-between px-5 py-3.5 hover:bg-[#dce6ff] transition-colors text-left">
...
@push('scripts')
<script>
// Smooth hover accordion — open on mouseenter, close on mouseleave
document.querySelectorAll('.skill-accordion-item').forEach(function(item) {
    var btn  = item.querySelector('.skill-acc-btn');
    var body = item.querySelector('.skill-accordion-body');
    var icon = item.querySelector('.skill-acc-icon');

    function openItem() {
        // Close all siblings first
        var siblings = item.closest('ul').querySelectorAll('.skill-accordion-item');
        siblings.forEach(function(sib) {
            if (sib !== item) {
                var sibBody = sib.querySelector('.skill-accordion-body');
                var sibIcon = sib.querySelector('.skill-acc-icon');
